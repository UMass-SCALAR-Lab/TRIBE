...
class OnlineDPOTrainer(_BaseTrainer):
    ...
    def _forward(self, model, prompt_ids, prompt_mask, completion_ids, completion_mask, vision_inputs=None):
        ...
        # Take the completion tokens logprob. selective_log_softmax is mathematically equivalent to
        # gather(logits.log_softmax(-1), ...) (see its own docstring) but never materializes the full
        # [batch, seq, vocab] log-softmax tensor -- the naive version above was the actual cause of this
        # trainer's OOMs at batch sizes every other trainer in this repo handles fine.
